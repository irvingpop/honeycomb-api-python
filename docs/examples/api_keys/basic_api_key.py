...
# start_example:list
async def list_api_keys(client: HoneycombClient, team: str) -> list[ApiKey]:
...
async def list_ingest_keys(client: HoneycombClient, team: str) -> list[ApiKey]:
    ...
    """
    keys = await client.api_keys.list_async(team, key_type="ingest")
...
# start_example:get
async def get_api_key(client: HoneycombClient, team: str, key_id: str) -> ApiKey:
    ...
    Returns:
        The API key object
    """
    key = await client.api_keys.get_async(team, key_id)
...
async def create_api_key(
    client: HoneycombClient, team: str, environment_id: str
) -> tuple[str, str]:
...
async def update_api_key(client: HoneycombClient, team: str, key_id: str, environment_id: str) -> ApiKey:
    ...
    Args:
        ...
        environment_id: Environment ID (required, must match existing)

    Returns:
        The updated API key
    """
    # Get existing key first to preserve values
    existing = await client.api_keys.get_async(team, key_id)
...
async def delete_api_key(client: HoneycombClient, team: str, key_id: str) -> None:
    """Delete an API key.
    ...
    """
    await client.api_keys.delete_async(team, key_id)
...
# CLEANUP
async def cleanup(client: HoneycombClient, team: str, key_id: str) -> None:
    """Clean up resources created by example."""
    await delete_api_key(client, team, key_id)
